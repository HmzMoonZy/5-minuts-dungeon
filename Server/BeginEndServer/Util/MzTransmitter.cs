...
namespace BeginEndServer.Util
{
    public static class MzTransmitter
    {
        ...
        #region ToPacket装包
        public static byte[] ToPacket(string msg)
        {
            byte[] msgBuff = Encoding.UTF8.GetBytes(msg);
            return ToPacket(msgBuff);
        }

        public static byte[] ToPacket(byte[] msgBuff)
        {
        ...
        #region UnPacket拆包
        /// <summary>
        /// 检查字节数组中是否有完整的Packet 若有则解析
        /// </summary>
        /// <param name="packet">需要拆的包字节数组</param>
        /// <param name="bufferCount">Conn 对象的读缓存区尾索引</param>
        /// <param name="msgLenth">包头显示的信息, 即数据总长度</param>
        /// <returns></returns>
        ...
        public static Message DeserializableMessage(byte[] msgBuff)
        {
            ...
            using (MemoryStream ms = new MemoryStream(msgBuff))
            {
                ...
                {
                    msg.OpCode = br.ReadInt32();    //4Byte
                    msg.SubCode = br.ReadInt32();   //4Byte
        ...
        public static byte[] MzMessageToBuffer(MzMessage msg)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                using (BinaryWriter bw = new BinaryWriter(ms))
        ...
        public static MzMessage BufferToMzMessage(byte[] msgBuffer)
        {
            MzMessage msg = new MzMessage();
